...
netDebug.style.cssText="position:fixed;left:14px;top:178px;z-index:50;background:rgba(0,0,0,.78);color:#8ff0b0;font:11px monospace;padding:6px 8px;border-radius:6px;pointer-events:none;max-width:300px;white-space:pre-wrap;display:none";
...
function fetchCharacterOnce(uid){
  // Resolves {ok:true, data} when Firebase genuinely answered (data may be
  // null if the player really has no save yet). Resolves {ok:false} if we
  // gave up waiting — a timeout is NOT the same as "no save exists" and
  // must never be treated as permission to overwrite the cloud save.
  return new Promise(resolve=>{
    let done=false;
    const finish=v=>{if(done)return;done=true;resolve(v)};
    const timer=setTimeout(()=>finish({ok:false}),15000);
    onValue(
      ref(db,`characters/${uid}`),
      snap=>{clearTimeout(timer);finish({ok:true,data:snap.val()})},
      err=>{clearTimeout(timer);console.warn("Cloud character read error:",err);finish({ok:false})},
      {onlyOnce:true}
    );
  });
}

async function loadCloudCharacter(user=currentUser,attempt=1){
  if(!user || cloudLoaded) return;
  try{
    const result=await fetchCharacterOnce(user.uid);
    if(!result.ok){
      // We don't actually know whether a cloud save exists — network was
      // too slow or errored. Do NOT call saveGame() here: that would wipe
      // a real save if one exists. Retry a few times before giving up.
      if(attempt<4){
        showMessage("Couldn't reach the cloud save yet — retrying...");
        setTimeout(()=>loadCloudCharacter(user,attempt+1),3000);
      }else{
        showMessage("Playing offline — couldn't reach the cloud save. Your progress is still saved locally.");
      }
      return;
    }
    cloudLoaded=true;
    ...
    if(data){
      ...
      // Firebase genuinely confirmed there's no character at this uid yet
      // (brand-new account) — safe to create the initial save.
      await saveGame();
    }
  }catch(e){
...
// Shared with setupTouchControls() further down — one detection rule used
// everywhere so button placement and the joystick always agree on whether
// this is a touch device.
function isTouchDevice(){
  return matchMedia("(pointer:coarse)").matches||("ontouchstart" in window&&!matchMedia("(hover:hover)").matches)||navigator.maxTouchPoints>0&&innerWidth<=900||innerWidth<=700||/[?&]touch\b/.test(location.search);
}
// Decided once, up front: on phones the HUD buttons are small icon-only
// circles (like the existing attack/talk/bag/stats buttons); on desktop
// they're the wider labeled pill. Avoids a full-width text button eating
// a big chunk of a narrow portrait screen.
const compactUI=isTouchDevice();
const btnShowDisplay=compactUI?"flex":"inline-flex";
...
addEventListener("keydown",e=>{
 ...
 if(k==="f"){toggleAutoFarm();e.preventDefault();return}
 if(k==="h"){toggleAutoHeal();e.preventDefault();return}
});
...
function movePlayer(){
 // Manual movement (real keyboard or the touch joystick, which sets the
 // same keys.w/a/s/d flags) always takes priority — grabbing the controls
 // hands movement back to the player and cancels auto-farm.
 if(autoFarm&&(keys.w||keys.a||keys.s||keys.d))stopAutoFarm("Auto-farm stopped (manual movement).");
 if(autoFarm&&autoFarmMove())return;
...
/* ================= AUTO-FARM =================
   Toggle with F (desktop) or the on-screen button (desktop + mobile).
   Locks onto whichever zone the player is standing in the moment it's
   turned on, and will only ever move toward / attack that zone's own mob
   type — it never crosses a zone boundary chasing something else. */
let autoFarm=false, autoFarmZone=null, autoFarmTargetId=null;
function setAutoFarmUI(){
  const btn=document.getElementById("autoFarmBtn");
  if(btn){
    btn.textContent=compactUI?"🚜":(autoFarm?"🚜 Auto-Farm: ON":"🚜 Auto-Farm: OFF");
    btn.style.background=autoFarm?"rgba(80,170,95,.9)":"rgba(20,25,36,.82)";
    btn.style.color=autoFarm?"#fff":"#e9c35b";
    btn.style.borderColor=autoFarm?"#8be09b":"#b89b52";
  }
  const t=document.getElementById("tFarm");
  if(t)t.style.background=autoFarm?"rgba(80,170,95,.9)":"rgba(20,25,36,.82)";
}
function stopAutoFarm(msg){
  autoFarm=false;autoFarmZone=null;autoFarmTargetId=null;
  setAutoFarmUI();
  if(msg)showMessage(msg);
}
function toggleAutoFarm(){
  if(!gameStarted||!currentUser){showMessage("Log in first.");return}
  if(autoFarm){stopAutoFarm("Auto-farm stopped.");return}
  const zone=zoneAt(player.x,player.y);
  if(!zone||!zone.mob){showMessage(`Nothing to farm in ${zone?zone.name:"this area"}.`);return}
  autoFarm=true;autoFarmZone=zone;autoFarmTargetId=null;
  setAutoFarmUI();
  showMessage(`Auto-farm started in ${zone.name} — hunting ${mobTypes[zone.mob].name}s only.`);
}
window.toggleAutoFarm=toggleAutoFarm;
// Returns true once it has taken over movement for this frame (either
// walking toward a target, or holding position while one respawns).
function autoFarmMove(){
  if(!autoFarmZone||!autoFarmZone.mob){stopAutoFarm();return false}
  // Re-validate every frame that the player is still inside the zone the
  // farm was started in — if something else moved them out (a quest
  // teleport scroll, defeat, etc.) auto-farm stops rather than dragging
  // them back in.
  if(zoneAt(player.x,player.y).id!==autoFarmZone.id){stopAutoFarm("Left the zone — auto-farm stopped.");return false}

  let target=autoFarmTargetId!=null?enemies.find(e=>e.id===autoFarmTargetId&&e.alive):null;
  if(!target){
    // Only ever consider mobs of this zone's own type — never mobs from
    // any other zone, even if one happened to be closer in a straight line.
    let best=null,bestD=Infinity;
    for(const e of enemies){
      if(!e.alive||e.type!==autoFarmZone.mob)continue;
      const d=Math.hypot(player.x-e.x,player.y-e.y);
      if(d<bestD){bestD=d;best=e}
    }
    target=best;
    autoFarmTargetId=target?target.id:null;
  }

  if(!target){
    // Everything in the zone is mid-respawn — hold position and wait.
    player.moving=false;
    return true;
  }
  ...
  if(d>60){
    ...
    player.direction=Math.abs(Math.cos(a))>Math.abs(Math.sin(a))?(Math.cos(a)>0?"right":"left"):(Math.sin(a)>0?"down":"up");
    const sp=derived().speed;
    const nx=player.x+Math.cos(a)*sp,ny=player.y+Math.sin(a)*sp;
    if(!blocked(nx,player.y))player.x=nx;
    if(!blocked(player.x,ny))player.y=ny;
    player.moving=true;
    if(++player.anim>6)player.anim=0;
  }else{
    player.moving=false;
    if(player.attackCooldown<=0)attack();
  }
  return true;
}

// Small on-screen toggle button. Works for both desktop (click) and mobile
// (tap) — created once here so no changes to index.html are required.
const autoFarmBtn=document.createElement("button");
autoFarmBtn.id="autoFarmBtn";
autoFarmBtn.type="button";
autoFarmBtn.textContent="🚜 Auto-Farm: OFF";
autoFarmBtn.style.cssText="all:unset;box-sizing:border-box!important;cursor:pointer;position:fixed!important;z-index:40!important;"+(isTouchDevice()
  ?"right:calc(14px + env(safe-area-inset-right,0px))!important;bottom:calc(300px + env(safe-area-inset-bottom,0px))!important;left:auto!important;top:auto!important;width:50px!important;height:50px!important;border-radius:50%!important;font-size:22px;display:flex!important;align-items:center;justify-content:center"
  :"left:calc(14px + env(safe-area-inset-left,0px))!important;top:calc(90px + env(safe-area-inset-top,0px))!important;width:auto!important;height:auto!important;max-width:200px;font:12px system-ui,sans-serif;padding:7px 12px;border-radius:8px;display:inline-flex!important;align-items:center"
 )+";background:rgba(20,25,36,.82);color:#e9c35b;border:1px solid #b89b52;display:none!important";
autoFarmBtn.addEventListener("click",()=>{initAudio();toggleAutoFarm()});
document.body.appendChild(autoFarmBtn);
setAutoFarmUI();

/* ================= AUTO-HEAL =================
   On by default. Every frame, if HP drops below the threshold and there's
   a healing consumable (e.g. Red Potion) in the inventory, it drinks one
   automatically via the same useIndex() path a manual click would use —
   same sfx, same message, same saveGame(). Toggle with H or the button. */
const AUTO_HEAL_HP_PCT=.5;
let autoHeal=true;
function setAutoHealUI(){
  const btn=document.getElementById("autoHealBtn");
  if(btn){
    btn.textContent=compactUI?"🧪":(autoHeal?"🧪 Auto-Heal: ON":"🧪 Auto-Heal: OFF");
    btn.style.background=autoHeal?"rgba(80,170,95,.9)":"rgba(20,25,36,.82)";
    btn.style.color=autoHeal?"#fff":"#e9c35b";
    btn.style.borderColor=autoHeal?"#8be09b":"#b89b52";
  }
}
function toggleAutoHeal(){
  autoHeal=!autoHeal;
  setAutoHealUI();
  showMessage(autoHeal?"Auto-heal enabled.":"Auto-heal disabled.");
}
window.toggleAutoHeal=toggleAutoHeal;
function autoHealCheck(){
  if(!autoHeal||!gameStarted||!currentUser||!cloudLoaded)return;
  if(player.hp<=0)return; // defeat() is already handling this
  const d=derived();
  if(player.hp>=d.maxHp*AUTO_HEAL_HP_PCT)return;
  const idx=inventory.findIndex(i=>i.cat==="consumable"&&i.stats?.heal);
  if(idx!==-1)useIndex(idx);
}
const autoHealBtn=document.createElement("button");
autoHealBtn.id="autoHealBtn";
autoHealBtn.type="button";
autoHealBtn.style.cssText="all:unset;box-sizing:border-box!important;cursor:pointer;position:fixed!important;z-index:40!important;"+(isTouchDevice()
  ?"right:calc(14px + env(safe-area-inset-right,0px))!important;bottom:calc(360px + env(safe-area-inset-bottom,0px))!important;left:auto!important;top:auto!important;width:50px!important;height:50px!important;border-radius:50%!important;font-size:22px;display:flex!important;align-items:center;justify-content:center"
  :"left:calc(14px + env(safe-area-inset-left,0px))!important;top:calc(134px + env(safe-area-inset-top,0px))!important;width:auto!important;height:auto!important;max-width:200px;font:12px system-ui,sans-serif;padding:7px 12px;border-radius:8px;display:inline-flex!important;align-items:center"
 )+";background:rgba(80,170,95,.9);color:#fff;border:1px solid #8be09b;display:none!important";
autoHealBtn.addEventListener("click",()=>{initAudio();toggleAutoHeal()});
document.body.appendChild(autoHealBtn);
setAutoHealUI();
function attack(){
...
function defeat(){sfx("defeat");player.hp=derived().maxHp;player.sp=derived().maxSp;player.x=18*TILE;player.y=18*TILE;player.gold=Math.max(0,player.gold-Math.floor(player.gold*.05));if(autoFarm)stopAutoFarm();showMessage("You were defeated and returned to Greenwood.");saveGame()}
...
function startGame(){
 ...
 authScreen.style.display="none";
 logoutBtn.style.display="block";
 autoFarmBtn.style.display=btnShowDisplay;
 autoHealBtn.style.display=btnShowDisplay;
 requestAnimationFrame(gameLoop);
}
function stopGame(){
 gameStarted=false;
 if(autoFarm)stopAutoFarm();
 autoFarmBtn.style.display="none";
 autoHealBtn.style.display="none";
 stopAmbient();
