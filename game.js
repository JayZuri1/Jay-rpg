...
function zoneAt(x,y){let tx=Math.floor(x/TILE),ty=Math.floor(y/TILE);return zones.find(z=>tx>=z.x1&&tx<=z.x2&&ty>=z.y1&&ty<=z.y2)||zones[0]}
...
addEventListener("keydown",e=>{
 ...
 if(e.key.toLowerCase()==="c")togglePanel("characterPanel");
 if(e.key.toLowerCase()==="m")toggleMap();
 if(["i","c","m"].includes(e.key.toLowerCase()))e.preventDefault();
});
...
function tone(f,d,type="sine",v=.05){initAudio();const o=audioContext.createOscillator(),g=audioContext.createGain();o.type=type;o.frequency.value=f;g.gain.setValueAtTime(v,audioContext.currentTime);g.gain.exponentialRampToValueAtTime(.001,audioContext.currentTime+d);o.connect(g);g.connect(audioContext.destination);o.start();o.stop(audioContext.currentTime+d)}
function sfx(kind){const a={attack:[480,.07,"sawtooth"],hit:[120,.08,"square"],coin:[880,.08,"square"],level:[784,.15,"sine"],hurt:[170,.12,"sawtooth"]}[kind];if(a)tone(...a)}
...
function attack(){
 if(player.attackCooldown>0)return;player.attackCooldown=derived().attackDelay;player.attacking=true;player.attackFrame=4;sfx("attack");
 let hit=false;
 for(const e of enemies)if(e.alive&&Math.hypot(player.x-e.x,player.y-e.y)<72){
   hit=true;
   if(Math.random()*100>derived().accuracy){showMessage("Miss!");continue}
   ...
   e.hp-=dmg;e.hitFlash=5;sfx("hit");
   floating(dmg,e.x,e.y,crit?"CRIT!":"");
   if(e.hp<=0)killMob(e);
 }
 if(!hit)showMessage("No target in range.");
}
function killMob(e){
...
function defeat(){player.hp=derived().maxHp;player.sp=derived().maxSp;player.x=18*TILE;player.y=18*TILE;player.gold=Math.max(0,player.gold-Math.floor(player.gold*.05));showMessage("You were defeated and returned to Greenwood.");saveGame()}
...
function interact(){
 ...
 if(near.type==="heal"){player.hp=derived().maxHp;player.sp=derived().maxSp;showMessage("Mira restored your HP and SP.");sfx("level");return}
 openQuestNPC(near);
}
function openQuestNPC(n){
 ...
 if(q&&q.done){player.gold+=q.gold;gainExp(q.exp);activeQuestId=null;showMessage(`Quest reward: ${q.gold} Gold + ${q.exp} EXP`);saveGame();return}
 if(!q){q=available[0]||questList.find(x=>!x.done);if(!q){showMessage("No quests available.");return}activeQuestId=q.id;showMessage(`Accepted: ${q.name}`)}
...
function buyItem(id){const i=item(id);if(player.gold<i.price){showMessage("Not enough gold.");return}player.gold-=i.price;give(id);showMessage(`Bought ${i.name}`);renderInventory();updateUI();saveGame()}
...
function useIndex(index){
 ...
 if(i.cat==="consumable"){if(i.stats.heal){player.hp=Math.min(derived().maxHp,player.hp+i.stats.heal);showMessage(`Used ${i.name}`)}else if(i.stats.sp){player.sp=Math.min(derived().maxSp,player.sp+i.stats.sp);showMessage(`Used ${i.name}`)}else if(i.stats.teleport){player.x=18*TILE;player.y=18*TILE;showMessage("Returned to Greenwood.")}inventory.splice(index,1);refreshPanels();fixVitals();saveGame()}
}
function unequip(slot){if(!equipment[slot])return;inventory.push(equipment[slot]);equipment[slot]=null;fixVitals();refreshPanels();saveGame()}
...
function showPanel(id){document.getElementById("panelBackdrop").style.display="block";document.getElementById(id).style.display="block"}
function closePanels(){document.querySelectorAll(".panel").forEach(p=>p.style.display="none");document.getElementById("panelBackdrop").style.display="none"}
...
function saveGame(){
 const data={player:{x:player.x,y:player.y,level:player.level,exp:player.exp,gold:player.gold,hp:player.hp,sp:player.sp,statPoints:player.statPoints,stats:player.stats},equipment,inventory,activeQuestId,quests:questList.map(q=>({id:q.id,progress:q.progress,done:q.done}))};
 localStorage.setItem("JAY_RPG_SAVE_V2",JSON.stringify(data))
}
function loadGame(){
 try{const d=JSON.parse(localStorage.getItem("JAY_RPG_SAVE_V2"));if(!d)return;Object.assign(player,d.player);Object.assign(equipment,d.equipment||{});inventory.length=0;(d.inventory||[]).forEach(i=>inventory.push(i));activeQuestId=d.activeQuestId??null;(d.quests||[]).forEach(q=>{const x=questList.find(a=>a.id===q.id);if(x){x.progress=q.progress;x.done=q.done}});fixVitals()}catch(e){console.warn("Save load failed",e)}
}
loadGame();

function toggleMap(){showMessage("The minimap is already visible in the bottom-right.");}
function gameLoop(){
 movePlayer();updateEnemies();if(player.attackCooldown>0)player.attackCooldown--;if(player.attacking&&--player.attackFrame<=0)player.attacking=false;updateCamera();updateFloats();ctx.clearRect(0,0,canvas.width,canvas.height);drawMap();drawDecorations();drawNPCs();drawEnemies();drawPlayer();drawFloats();drawMinimap();updateUI();requestAnimationFrame(gameLoop)
}
